...
class MultiViewModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if opt.use_pyramid:
            self.netG = FTAE_pyramid(input_nc, opt.ngf, n_layers=int(np.log2(opt.fineSize)), upsample=opt.upsample,
                         norm_layer=networks.get_norm_layer(norm_type=opt.norm),
                         nl_layer=networks.get_non_linearity(layer_type='lrelu'), gpu_ids=opt.gpu_ids,
                         nz=opt.nz, use_vae=opt.use_vae)
        else:
            self.netG = FTAE(input_nc, opt.ngf, n_layers=int(np.log2(opt.fineSize)), upsample=opt.upsample,
                         norm_layer=networks.get_norm_layer(norm_type=opt.norm),
                         nl_layer=networks.get_non_linearity(layer_type='lrelu'), gpu_ids=opt.gpu_ids,
                         nz=opt.nz, use_vae=opt.use_vae)
    ...
    def test(self):
        ...
        for i in range(20):
            if self.opt.concat_grid:
                ...
                real_A_grid = self.real_A
            fake_B_flow = self.netG(real_A_grid, Variable(torch.Tensor([-1/4.*np.pi + 1/2.*np.pi*i/19 ]).cuda(self.gpu_ids[0], async=True)).unsqueeze(0))
