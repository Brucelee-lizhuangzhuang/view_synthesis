...
class remap_flow(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, map, grid):
        ff2bf_map = map.cpu().numpy().astype(np.uint8) # [NxCxHxW]
        ff2bf_map = np.clip(ff2bf_map,0,127)
        bf2ff_map = np.zeros(ff2bf_map.shape)

        bf2ff_map[:,ff2bf_map[:,:,:,1], ff2bf_map[:,:,:,0],:] = grid
        ctx.bf2ff_map = bf2ff_map.astype(np.uint8)
        bf2ff_map = bf2ff_map / 64. - 1.
        return torch.from_numpy(bf2ff_map.astype(np.float32)).cuda()
    @staticmethod
    def backward(ctx, grad_output):
#         print(grad_output)
        grad_output = grad_output.data.cpu().numpy() # [NxCxHxW]
        ff2bf_map = np.zeros(grad_output.shape)
        bf2ff_map = ctx.bf2ff_map
        ff2bf_map[:,bf2ff_map[:,:,:,1], bf2ff_map[:,:,:,0],:] = grad_output
        return Variable(torch.from_numpy(ff2bf_map.astype(np.float32)).cuda()), None, None

class flow_remapper(nn.Module):
    def __init__(self, size=128, batch=1, gpu_ids=[]):
        super(flow_remapper, self).__init__()
        self.gpu_ids = gpu_ids
        self.size = size
        ...
        for i in range(grid.shape[0]):
            for j in range(grid.shape[1]):
                grid[i,j,0] = j
                grid[i,j,1] = i
        self.grid = np.reshape(grid, (1,128,128,2))
        self.grid = np.repeat(self.grid,batch, axis=0)
    def forward(self, input, map):
        return remap_flow.apply( (input+1)*self.size/2.,  (map+1)*self.size/2., self.grid)